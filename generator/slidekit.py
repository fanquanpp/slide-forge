# -*- coding: utf-8 -*-
"""SlideForge 渲染引擎：形状 / 文本工具 + 14 种可复用版式原型。
...
class Deck:
    ...
    def rect(self, s, l, tp, w, h, fill=None, line=None, lw=0.75,
             radius=0.0, alpha=None, shape=MSO_SHAPE.RECTANGLE):
    ...
    def text(self, s, txt, l, tp, w, h, size=18, color="1A1A1A", bold=False,
             font=None, ea=None, align=PP_ALIGN.LEFT, anchor=MSO_ANCHOR.TOP,
             spacing=1.0, italic=False, space_after=4, wrap=True):
        tf_font = font if font is not None else self.t["title_font"][0]
        tf_ea = ea if ea is not None else self.t["title_font"][1]
        ...
        for i, ln in enumerate(lines):
            ...
            f.color.rgb = C(color); f.name = tf_font
            _set_ea(r, tf_ea)
        return tb

    def bullets(self, s, items, l, tp, w, h, size=16, color=None, accent=None,
                gap=10, dot=True):
        color = color or self.t["ink2"]
    ...
    def pic_ph(self, s, l, tp, w, h, label="图片占位", tone=None):
        ...
        try:
            ...
            pass
        self.text(s, label, l, tp + h / 2 - 0.18, w, 0.36, size=12,
                  color=self.t["ink2"], align=PP_ALIGN.CENTER, anchor=MSO_ANCHOR.MIDDLE,
    ...
    def header(self, s, kicker, title, accent=None):
        accent = accent or self.t["accent"]
        self.text(s, kicker.upper(), 0.9, 0.62, 8.5, 0.3, size=11.5, color=accent,
                  bold=True, font=self.t["body_font"][0], ea=self.t["body_font"][1])
        self.text(s, title, 0.9, 0.95, 11.5, 0.6, size=28, color=self.t["ink"], bold=True)
        self.rect(s, 0.9, 1.72, 0.62, 0.055, accent)

    def footer(self, s, idx=None):
        self.rect(s, 0.9, SH - 0.62, SW - 1.8, 0.008, self.t["line"])
        self.text(s, self.t["label"] + "  ·  " + self.t["en"], 0.9, SH - 0.55, 7.0, 0.3,
                  size=9, color=self.t["ink2"], font=self.t["body_font"][0], ea=self.t["body_font"][1])
        if idx is not None:
            self.text(s, "%02d" % idx, SW - 1.9, SH - 0.55, 1.0, 0.3, size=9,
                      color=self.t["ink2"], align=PP_ALIGN.RIGHT,
                      font=self.t["body_font"][0], ea=self.t["body_font"][1])
        self._page = idx or self._page

    # ---------- 主题装饰 ----------
    def decor(self, s, variant="cover"):
        d = self.t.get("decor", "min")
        a, a2 = self.t["accent"], self.t["accent2"]
        if d == "grid":
            for x in range(1, 13):
                self.rect(s, x, 0, 0.006, SH, self.t["line"])
            self.rect(s, 0.9, 0, 0.05, SH, a)
        elif d == "band":
            self.rect(s, 0, 0, SW, 0.16, a)
            self.rect(s, 0, 0.16, SW, 0.06, a2)
        elif d == "glow":
            self.oval(s, 9.6, -1.6, 6.2, 6.2, a, alpha=0.16)
            self.oval(s, -1.8, 4.6, 5.4, 5.4, a2, alpha=0.13)
        elif d == "pixel":
            for i in range(6):
            ...
            self.oval(s, 9.0, -1.4, 6.0, 6.0, a, alpha=0.20)
            self.oval(s, 8.2, 2.6, 7.2, 7.2, a2, alpha=0.14)
        elif d == "term":
            self.text(s, "$", 12.2, 0.6, 0.6, 0.5, size=18, color=a, font=self.t["body_font"][0])
            self.rect(s, 12.75, 0.72, 0.14, 0.28, a)
        elif d == "organic":
            self.oval(s, 10.6, -1.2, 3.6, 3.6, a, alpha=0.20)
            self.oval(s, 11.6, 2.2, 2.6, 2.6, a2, alpha=0.22)
        elif d == "soft":
    ...
    def cover(self, title="在此输入演示标题", subtitle="在此输入副标题 · 团队 / 姓名",
        ...
        if variant == "left":
            ...
            self.text(s, subtitle, 0.9, 4.15, 10.0, 0.6, size=18, color=self.t["ink2"],
                      font=self.t["body_font"][0], ea=self.t["body_font"][1])
            self.text(s, meta, 0.9, 5.9, 8.0, 0.4, size=12, color=self.t["ink2"],
                      font=self.t["body_font"][0], ea=self.t["body_font"][1])
        elif variant == "center":
            self.text(s, title, 1.2, 2.6, 10.9, 1.6, size=46, bold=True, color=self.t["ink"],
                      align=PP_ALIGN.CENTER, spacing=1.02)
            self.rect(s, SW/2 - 0.45, 4.25, 0.9, 0.1, a)
            self.text(s, subtitle, 1.2, 4.5, 10.9, 0.6, size=18, color=self.t["ink2"],
                      align=PP_ALIGN.CENTER, font=self.t["body_font"][0], ea=self.t["body_font"][1])
            self.text(s, meta, 1.2, 6.2, 10.9, 0.4, size=12, color=self.t["ink2"],
                      align=PP_ALIGN.CENTER, font=self.t["body_font"][0], ea=self.t["body_font"][1])
        else:  # split
            self.rect(s, 0, 0, 5.4, SH, a, alpha=0.95)
            self.text(s, "SECTION\n01", 0.8, 2.4, 4.0, 1.6, size=40, bold=True, color="FFFFFF", spacing=1.05)
            self.text(s, title, 6.0, 2.5, 6.6, 2.0, size=38, bold=True, color=self.t["ink"], spacing=1.03)
            self.text(s, subtitle, 6.0, 4.5, 6.4, 0.7, size=17, color=self.t["ink2"],
                      font=self.t["body_font"][0], ea=self.t["body_font"][1])
        return s
    ...
    def agenda(self, items=None, title="目录  CONTENTS", variant="num"):
        ...
        if variant == "num":
            ...
            for i, it in enumerate(items):
                cx = 0.9 + (i % cols) * 6.0
                cy = 2.35 + (i // cols) * rowh
                self.text(s, "%02d" % (i + 1), cx, cy, 0.8, 0.5, size=18, bold=True, color=self.t["accent"],
                          font=self.t["body_font"][0], ea=self.t["body_font"][1])
    ...
    def section(self, num="01", title="章节标题", desc="章节说明 · 一句话点题"):
        ...
        self.text(s, desc, 0.9, 5.15, 8.6, 0.6, size=15, color=self.t["ink2"],
                  font=self.t["body_font"][0], ea=self.t["body_font"][1])
        self.footer(s)
        return s
    ...
    def bullets_page(self, kicker, title, items, variant="dot"):
        ...
        else:  # two-digit number + text
            for i, it in enumerate(items[:5]):
                cy = 2.25 + i * 0.86
                self.text(s, "%02d" % (i + 1), 0.9, cy, 1.0, 0.6, size=22, bold=True, color=self.t["accent"])
                self.text(s, it, 2.1, cy + 0.05, 10.0, 0.5, size=16, color=self.t["ink"],
                          font=self.t["body_font"][0], ea=self.t["body_font"][1])
                self.line(s, 2.1, cy + 0.66, 12.4, cy + 0.66, self.t["line"], 0.6)
        self.footer(s)
        return s

    def two_col(self, kicker, title, left_title="左侧标题", right_title="右侧标题",
                left=None, right=None, variant="compare"):
        left = left or ["在此输入要点一", "在此输入要点二", "在此输入要点三"]
        right = right or ["在此输入要点一", "在此输入要点二", "在此输入要点三"]
        s = self.slide(); self.bg(s); self.header(s, kicker, title)
        cols = [(0.9, left_title, left, self.t["accent"], "A"), (6.95, right_title, right, self.t["accent2"], "B")]
        for cx, ct, items, ac, tag in cols:
        ...
        if variant == "vs":
            ...
            self.text(s, "VS", SW/2 - 0.42, 4.05, 0.84, 0.84, size=14, bold=True, color=self.t["bg"],
                      align=PP_ALIGN.CENTER, anchor=MSO_ANCHOR.MIDDLE, font=self.t["body_font"][0], ea=self.t["body_font"][1])
        self.footer(s)
        return s
    ...
    def cards(self, kicker, title, cards_data, cols=4):
        s = self.slide(); self.bg(s); self.header(s, kicker, title)
        n = len(cards_data)
        cols = min(cols, max(1, n)); rows = (n + cols - 1) // cols
        gap = 0.3
        cw = (SW - 1.8 - (cols - 1) * gap) / cols
        ch = (4.25 - (rows - 1) * 0.3) / rows
        for i, (val, lab, desc) in enumerate(cards_data):
            r, c = divmod(i, cols)
            cx = 0.9 + c * (cw + gap); cy = 2.3 + r * (ch + 0.3)
            ...
            self.text(s, val, cx + 0.35, cy + 0.62, cw - 0.7, ch * 0.42, size=30, bold=True, color=self.t["accent"])
            self.text(s, lab, cx + 0.35, cy + ch * 0.66, cw - 0.7, 0.4, size=14, bold=True, color=self.t["ink"])
            self.text(s, desc, cx + 0.35, cy + ch * 0.82, cw - 0.7, ch * 0.3, size=11, color=self.t["ink2"],
                      font=self.t["body_font"][0], ea=self.t["body_font"][1])
        self.footer(s)
        return s
    ...
    def timeline(self, kicker, title, steps):
        ...
        for i, (st, de) in enumerate(steps):
            ...
            self.text(s, "%02d" % (i + 1), cx - 0.4, y - 0.62 if up else y + 0.1, 0.8, 0.3,
                      size=11, bold=True, color=self.t["accent"], align=PP_ALIGN.CENTER)
            self.text(s, st, cx - 1.0, ty, 2.0, 0.4, size=15, bold=True, color=self.t["ink"], align=PP_ALIGN.CENTER)
            self.text(s, de, cx - 1.0, ty + 0.42, 2.0, 0.8, size=11, color=self.t["ink2"],
                      align=PP_ALIGN.CENTER, font=self.t["body_font"][0], ea=self.t["body_font"][1], spacing=1.05)
        self.footer(s)
        return s
    ...
    def process(self, kicker, title, nodes, style="chevron"):
        ...
        for i, nd in enumerate(nodes):
            ...
            self.text(s, nd, cx + 0.25, 3.55, cw - 0.5, 0.5, size=14.5, bold=True, color=ink,
                      align=PP_ALIGN.CENTER, wrap=True)
            if i < n - 1:
                self.text(s, "›", cx + cw + 0.02, 3.55, gap - 0.04, 0.5, size=18, color=self.t["ink2"],
    ...
    def table_ph(self, kicker, title, headers, rows):
        s = self.slide(); self.bg(s); self.header(s, kicker, title)
        cols = len(headers); x0, y0 = 0.9, 2.35
        tw = SW - 1.8; rowh = min(0.7, 4.0 / (len(rows) + 1)); cw = tw / cols
        self.rect(s, x0, y0, tw, rowh, self.t["accent"])
        for j, hd in enumerate(headers):
            self.text(s, hd, x0 + j * cw + 0.2, y0 + rowh * 0.2, cw - 0.4, rowh * 0.6, size=13,
                      bold=True, color="FFFFFF", font=self.t["body_font"][0], ea=self.t["body_font"][1])
        for i, row in enumerate(rows):
            ...
            for j, cell in enumerate(row):
                self.text(s, cell, x0 + j * cw + 0.2, cy + rowh * 0.2, cw - 0.4, rowh * 0.6, size=12.5,
                          color=self.t["ink"], font=self.t["body_font"][0], ea=self.t["body_font"][1])
        self.footer(s)
        return s
    ...
    def chart_ph(self, kicker, title, variant="bar"):
        ...
        if variant == "bar":
            vals = [0.55, 0.8, 0.45, 0.95, 0.7, 0.6]
            for i, v in enumerate(vals):
                bh = 3.2 * v; cx = 1.5 + i * 1.25
                self.rect(s, cx, 6.0 - bh, 0.75, bh, self.t["accent"] if i % 2 == 0 else self.t["accent2"], alpha=0.9)
        elif variant == "line":
            ...
            self.text(s, "图", 4.5, 3.9, 1.0, 1.0, size=16, bold=True, color=self.t["accent"],
                      align=PP_ALIGN.CENTER, anchor=MSO_ANCHOR.MIDDLE, font=self.t["body_font"][0], ea=self.t["body_font"][1])
        self.text(s, "图表占位 · 在 PowerPoint 中插入图表替换", 1.4, 6.15, 6.0, 0.3, size=10.5, color=self.t["ink2"],
                  font=self.t["body_font"][0], ea=self.t["body_font"][1])
    ...
    def gallery(self, kicker, title, cols=3):
        s = self.slide(); self.bg(s); self.header(s, kicker, title)
        rows = 2; gap = 0.3
        cw = (SW - 1.8 - (cols - 1) * gap) / cols
        ch = (4.25 - (rows - 1) * gap) / rows
        for i in range(cols * rows):
            r, c = divmod(i, cols)
            cx = 0.9 + c * (cw + gap); cy = 2.3 + r * (ch + gap)
    ...
    def quote(self, quote_text, author="— 署名 / 职位"):
        ...
        self.text(s, author, 1.4, 5.4, 8.0, 0.5, size=15, color=self.t["ink2"],
                  font=self.t["body_font"][0], ea=self.t["body_font"][1])
        self.footer(s)
        return s
    ...
    def team(self, kicker, title, members):
        s = self.slide(); self.bg(s); self.header(s, kicker, title)
        members = members or [("姓名", "角色"), ("姓名", "角色"), ("姓名", "角色"), ("姓名", "角色")]
        n = len(members); gap = 0.35
        cw = (SW - 1.8 - (n - 1) * gap) / n
        for i, (nm, ro) in enumerate(members):
            ...
            self.text(s, "头像", cx + cw / 2 - 0.65, 2.95, 1.3, 1.3, size=11, color=self.t["ink2"],
                      align=PP_ALIGN.CENTER, anchor=MSO_ANCHOR.MIDDLE, font=self.t["body_font"][0], ea=self.t["body_font"][1])
            self.text(s, nm, cx + 0.2, 4.5, cw - 0.4, 0.4, size=15, bold=True, color=self.t["ink"], align=PP_ALIGN.CENTER)
            self.text(s, ro, cx + 0.2, 4.95, cw - 0.4, 0.4, size=12, color=self.t["accent"], align=PP_ALIGN.CENTER,
                      font=self.t["body_font"][0], ea=self.t["body_font"][1])
        self.footer(s)
        return s
    ...
    def closing(self, title="谢谢观看", sub="在此输入联系方式 / 二维码 / 致谢"):
        s = self.slide(); self.bg(s); self.decor(s, "cover")
        self.text(s, title, 1.2, 2.9, 10.9, 1.2, size=48, bold=True, color=self.t["ink"],
                  align=PP_ALIGN.CENTER)
        self.rect(s, SW/2 - 0.45, 4.3, 0.9, 0.1, self.t["accent"])
        self.text(s, sub, 1.2, 4.6, 10.9, 0.7, size=16, color=self.t["ink2"],
                  align=PP_ALIGN.CENTER, font=self.t["body_font"][0], ea=self.t["body_font"][1])
        self.footer(s)
        return s
